...
class Test(object):
    ...
    def accuracy(self, map_path=None):
        with torch.no_grad():
            ...
            msg = '%s MAE=%.6f, F-score=%.6f, len(imgs)=%s, fps=%.4f' % (
            self.datapath, mae / cnt, fscore.max() / cnt, len(self.loader.dataset), fps)
            print(msg)
            logger.info(msg)
...
EXP_NAME = 'trained'
JSON_METHOD = 'cod_method.json'
JSON_DATA = 'cod_dataset.json'

if __name__ == '__main__':
    # set torch cuda device
    cfg = dataset.Config(datapath='000', mode='test')
    net = Net(cfg)
    path = './out//trained//model-best.pth'
