...
class Data(Dataset):
    def __init__(self, cfg):
        ...
        with open(cfg.datapath+'/'+cfg.mode+'.txt', 'r') as lines:
            ...
            #       # print(f'gtpath:{maskpath}')
            #       # print(cfg.label_dir)
            #       self.samples.append([imagepath, maskpath])
            # else:
            #   for line in lines:
            #       imagepath = cfg.datapath + '/' + cfg.mode + '/' + line.strip() +  '.jpg'
            #       maskpath = cfg.datapath + '/' + cfg.mode + '/' + line.strip().split('/')[0] + '/GT/' + line.strip().split('/')[2] + '.png'
            #
            #       # imagepath = cfg.datapath.split('/')[0] + '/' + cfg.mode + '/' + line.strip() + '.jpg'
            #       # maskpath = cfg.datapath.split('/')[0] + '/' + cfg.mode + '/' + line.strip().split('/')[0] + '/GT/' + line.strip().split('/')[2] + '.png'
            #
            #       # imagepath = cfg.datapath.split('/')[0] + '/' + cfg.mode + '/' + line.replace(" ","") + '.jpg'
            #       # maskpath = cfg.datapath.split('/')[0] + '/' + cfg.mode + '/' + line.replace(" ","").split('/')[0] + '/GT/' + line.replace(" ","").split('/')[2] + '.png'
            #       # print(f'imgpath:{imagepath}')
            #       # print(f'maskpath:{maskpath}')
            #       #maskpath  = cfg.datapath + '/GT/' + line.strip().split('/')[2]+  '.png'
            #       self.samples.append([imagepath, maskpath])
            if cfg.mode == 'train':
                for line in lines:
                    # print(line)
...
if __name__=='__main__':
    ...
    plt.ion()
    cfg  = Config(mode='test', datapath='/home/dxxy/jiaxu/PythonProject/COD1/CodDataset')
    data = Data(cfg)
